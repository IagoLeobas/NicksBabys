package br.unitins.usuario.filter;
...
//		return;
		...
		if (usuario == null) {
			((HttpServletResponse) response).sendRedirect("/NicksBabys/login.com");
		} else {
			if (usuario.getTipoUsuario().getPaginasComPermissao().contains(endereco)) {
				chain.doFilter(request, response);
				return;
			} else
				((HttpServletResponse) response).sendRedirect("/NicksBabys/sempermissao.xhtml");
		}
